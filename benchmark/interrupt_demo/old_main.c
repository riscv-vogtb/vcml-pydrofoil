...
static inline void csr_skip_faulting_instruction(void)
{
    uintptr_t mepc;
    asm volatile("csrr %0, mepc" : "=r"(mepc));

    uint16_t instr = *(volatile uint16_t *)mepc;
    uintptr_t advance = ((instr & 0x3) == 0x3) ? 4 : 2;

    asm volatile("csrw mepc, %0" ::"r"(mepc + advance));
}
...
// code 5 = Load access fault (this is the memory/shadow-mem trap)
...
void trap_dispatch(void)
{
    ...
    {
        uart_print("Memory trap received\n");
        mem_trap_handler();
